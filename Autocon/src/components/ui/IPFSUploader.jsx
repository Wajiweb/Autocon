export default function IPFSUploader({ metadataState, handleFileUpload, handleUpdateMetadata, setMetadataState, setFormData }) {
  return (
    <div style={{ marginBottom: '24px' }}>
      ...
      <div className={`p-5 rounded-[var(--radius-lg)] border-2 transition-all duration-200 ${metadataState.status === 'ready' ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-[var(--border-dark)] border-dashed bg-[rgba(255,255,255,0.04)]'}`}>
        {metadataState.status === 'idle' || metadataState.status === 'error' ? (
          ...
          <div className="flex flex-col gap-4">
            <div className="flex gap-4 items-start">
              <div className="w-20 h-20 rounded-xl bg-white overflow-hidden border border-[var(--border-light)] shrink-0">
              ...
              <div className="flex-1">
                ...
                <input
                  ...
                  className="w-full bg-white text-[var(--text-primary)] border border-[var(--border-light)] rounded-lg px-3 py-2 text-xs mb-2 outline-none focus:border-[var(--primary)]"
                  placeholder="NFT Name"
                />
                <input
                  value={metadataState.description}
                  onChange={e => setMetadataState(prev => ({ ...prev, description: e.target.value }))}
                  className="w-full bg-white text-[var(--text-primary)] border border-[var(--border-light)] rounded-lg px-3 py-2 text-xs outline-none focus:border-[var(--primary)]"
