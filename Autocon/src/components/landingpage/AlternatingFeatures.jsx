...
const CryptoMockupOne = () => (
  <div style={{ width: '100%', height: '100%', background: 'var(--surface)', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    <div style={{ position: 'relative', background: 'var(--surface)', border: '1px solid #ddd', padding: '24px', borderRadius: '16px', width: '70%' }}>
      ...
      <div style={{ color: 'var(--text-muted)', fontFamily: 'monospace', fontSize: '0.85rem' }}>
...
const features = [
  {
    icon: <FileCode2 size={24} color="var(--surface)" />, title: 'Future-Forward Contract Generation',
  ...
  {
    icon: <ShieldAlert size={24} color="var(--surface)" />, title: 'Efficient Smart Contract Security',
  ...
  {
    icon: <Cpu size={24} color="var(--surface)" />, title: 'Intelligent Web3 Assistance',
...
const AlternatingFeatures = ({ onLearnMore }) => (
  <section id="features" style={{ background: 'var(--surface)', padding: '0 48px 32px' }}>
    {features.map((f, i) => (
      <Reveal key={f.title} delay={0.1}>
        <div style={{
          ...
          <div style={{ order: i % 2 !== 0 ? 2 : 1 }}>
            ...
            <p style={{ color: 'var(--text-muted)', lineHeight: 1.7, fontSize: '0.875rem', marginBottom: '20px' }}>
              {f.desc}
            </p>
            <button
              onClick={onLearnMore}
              style={{
                ...
                background: 'var(--primary)', color: 'var(--surface)', fontSize: '0.8rem', fontWeight: 700,
                cursor: 'pointer',
              }}
